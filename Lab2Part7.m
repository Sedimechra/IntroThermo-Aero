...
for i = 1:31
    ...
    for j = 1:17
        ...
        Ca1=Ca1+(0.5*(Cp_Vel1(j+1,i)+Cp_Vel1(j+2,i))*((xPlot(j+1)-yPlot(j))/3.5));
        Ca2=Ca2+(0.5*(Cp_Vel2(j+1,i)+Cp_Vel2(j+2,i))*((xPlot(j+1)-yPlot(j))/3.5));
        Ca3=Ca3+(0.5*(Cp_Vel3(j+1,i)+Cp_Vel3(j+2,i))*((xPlot(j+1)-yPlot(j))/3.5));
    end
    Cp_Vel1(21,i)=-Cn1;
    Cp_Vel2(21,i)=-Cn2;
    Cp_Vel3(21,i)=-Cn3;
...
title('Cp vs x')
nexttile
plot(Cp_Vel2(20,:),Cp_Vel2(23,:));
title('Cl vs AOA')
nexttile
plot(Cp_Vel2(20,:),Cp_Vel2(24,:));
title ('Cd vs AOA')
